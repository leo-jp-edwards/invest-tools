...
class Portfolio:
    ...
    def build(self, portfolio_definition: typing.Dict[str, typing.Dict[str, str]]) -> pd.DataFrame:
        """
        ...
        for code, opts in portfolio_definition.items():
            ...
            if opts["currency"] != 'gpb':
                ret = self.calculate_returns(self.prices, code, convert=True, cur=self.gbpusd)
            else:
                ret = self.calculate_returns(self.prices, code, convert=False, cur=self.gbpusd)
    ...
    def _get_data(self) -> typing.List[pd.DataFrame]:
        ...
        This data can also just be stored in a `data` directory at the same level as the file that calls this package.
        """
    ...
    def get_usd_converter(self, conversion_csv: str) -> pd.DataFrame:
        """
        Get a dataframe of USD to GBP to convert the prices between currencies. All portfolio prices
        and returns should be in GBP.
    ...
    def calculate_returns(self, prices: pd.DataFrame, code: str, convert: bool, cur: pd.DataFrame) -> pd.DataFrame:
        """
        Take in a dataframe of prices of all codes and calculate the returns shown by each
        security and then record those returns into a single dataframe.
        ...
        The function will work by filtering the input frame by each TIDM and calculating the adjusted
        and converted returns and appending them to a new dataframe.

        :returns: A dataframe of a single TIDM with returns adjusted by adjustment and converted to currency
        """
        ti = prices.loc[
            prices.TIDM == code
        ]
